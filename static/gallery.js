...
document.addEventListener('DOMContentLoaded', () => {
                    ...
                    if (cellModelConfig && cellModelConfig.name === modelName && cellPrompt === prompt) {
                        ...
                        data.urls.forEach((url, index) => {
                            if (links[index]) {
                                ...
                                if (img) {
                                    // Extract UUID from URL: /image/200/75/{uuid}.webp
                                    const uuidMatch = url.match(/\/image\/200\/75\/(.+)\.webp$/);
                                    if (uuidMatch) {
                                        const uuid = uuidMatch[1];
                                        // Build srcset with responsive variants
                                        const srcset = `/image/200/75/${uuid}.webp 1x, /image/400/85/${uuid}.webp 2x, /image/600/90/${uuid}.webp 3x`;
                                        img.srcset = srcset;
                                    }
    ...
    function getFullResolutionUrl(thumbnailUrl) {
        ...
        // Example: /image/200/75/{uuid}.webp -> /image/1.0/90/{uuid}.webp
        const parts = thumbnailUrl.split('/');
        if (parts.length >= 5 && parts[0] === '' && parts[1] === 'image') {
            // Replace size (index 2) and quality (index 3), keep uuid.webp (index 4)
