/// Minimal example demonstrating error chain loss with thiserror errors
...
enum ComfyUIError {
    ...
    _ServerError { status: u16, message: String },

    #[error("Checkpoint '{name}' not found in model directory")]
    CheckpointNotFound { name: String },
}

#[derive(Debug, Error)]
enum ModelError {
    #[error("Failed to load model from disk")]
    LoadFailed(#[source] ComfyUIError),

    #[error("Model validation failed")]
    _ValidationFailed(#[source] ComfyUIError),
}
...
impl Message<DoWork> for Worker {
    ...
    async fn handle(
        ...
        // Simulate a deep error chain with thiserror types
        let checkpoint_error = ComfyUIError::CheckpointNotFound {
            name: "xl/animavividXL_v10.safetensors".to_string(),
        };

        let model_error = ModelError::LoadFailed(checkpoint_error);

        // Convert to anyhow and add context
        Err(anyhow::Error::from(model_error)).context("while initializing ComfyUI backend")
    }
}
...
async fn main() -> Result<()> {
    println!("Kameo Error Chain Loss with thiserror - Minimal Example");
    println!("{}", "=".repeat(60));
    ...
    println!("\n{}", "=".repeat(60));
    println!("Notice: Test 1 loses the checkpoint details!");
    println!("        Test 2 preserves the full thiserror error chain.");
