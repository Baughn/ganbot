...
impl Generate {
    pub fn from_string(raw: &str) -> Result<Generate> {
            ...
            match token {
                ...
                _ if token.starts_with('-') && !token.starts_with("--") && token.len() > 2 => {
                    ...
                    let looks_like_combined = match flag {
                        "-c" | "-w" | "-h" | "-s" => value.chars().next().map_or(false, |c| c.is_ascii_digit()),
...
fn parse_aspect_ratio(s: &str) -> Result<(u32, u32)> {
    ...
    bail!("Invalid aspect ratio format: {}. Use formats like 16:9, 16x9, or 16/9", s)
}
...
mod tests {
    ...
    fn test_simple_prompt() {
        let result = Generate::from_string("a beautiful sunset over the ocean").unwrap();
    ...
    fn test_prompt_with_model() {
        let result = Generate::from_string("a cat -m flux").unwrap();
        assert_eq!(result.prompt, "a cat");
        assert_eq!(result.model, Some("flux".to_string()));
        
        let result2 = Generate::from_string("a dog --model dalle3").unwrap();
    ...
    fn test_prompt_with_dimensions() {
        let result = Generate::from_string("landscape -w 1024 -h 768").unwrap();
        assert_eq!(result.prompt, "landscape");
        assert_eq!(result.width, Some(1024));
        assert_eq!(result.height, Some(768));
        
        let result2 = Generate::from_string("portrait --width 512 --height 1024").unwrap();
    ...
    fn test_prompt_with_aspect_ratio() {
        let result = Generate::from_string("wide image --ar 16:9").unwrap();
        assert_eq!(result.prompt, "wide image");
        assert_eq!(result.aspect, Some((16, 9)));
        
        let result2 = Generate::from_string("square --aspect 1x1").unwrap();
        assert_eq!(result2.prompt, "square");
        assert_eq!(result2.aspect, Some((1, 1)));
        
        let result3 = Generate::from_string("tall --ar 9/16").unwrap();
    ...
    fn test_prompt_with_negative() {
        let result = Generate::from_string("beach scene --no umbrella chairs people").unwrap();
        assert_eq!(result.prompt, "beach scene");
        assert_eq!(result.negative_prompt, Some("umbrella chairs people".to_string()));
    }

    #[test]
    fn test_prompt_with_multiple_options() {
        let result = Generate::from_string("cyberpunk city -m flux -w 1024 -h 768 --seed 42 -s 30 --no blur").unwrap();
    ...
    fn test_combined_short_options() {
        let result = Generate::from_string("test -w512 -h256").unwrap();
    ...
    fn test_long_options_with_equals() {
        let result = Generate::from_string("test --width=1920 --height=1080 --model=sdxl").unwrap();
        assert_eq!(result.prompt, "test");
    ...
    fn test_count_option() {
        let result = Generate::from_string("multiple cats -c 4").unwrap();
        assert_eq!(result.prompt, "multiple cats");
        assert_eq!(result.num_images, Some(4));
        
        let result2 = Generate::from_string("batch --count 10").unwrap();
    ...
    fn test_empty_prompt_with_negative() {
        ...
        let result = Generate::from_string("--no blurry distorted").unwrap();
    ...
    #[test]
    fn test_empty_input() {
        let result = Generate::from_string("");
        assert!(result.is_err());
        
        let result2 = Generate::from_string("   ");
        assert!(result2.is_err());
    }

    #[test]
    fn test_invalid_numeric_values() {
        let result = Generate::from_string("test -w abc");
        assert!(result.is_err());
        
        let result2 = Generate::from_string("test --seed notanumber");
        assert!(result2.is_err());
        
        let result3 = Generate::from_string("test -s -10");
        assert!(result3.is_err());
    }

    #[test]
    fn test_invalid_aspect_ratio() {
        let result = Generate::from_string("test --ar 16");
        assert!(result.is_err());
        
        let result2 = Generate::from_string("test --ar 0:9");
        assert!(result2.is_err());
        
        let result3 = Generate::from_string("test --ar abc:def");
        assert!(result3.is_err());
    }

    #[test]
    fn test_missing_option_values() {
        let result = Generate::from_string("test -w");
        assert!(result.is_err());
        
        let result2 = Generate::from_string("test --model");
        assert!(result2.is_err());
    }

    #[test]
    fn test_unknown_options_treated_as_prompt() {
        let result = Generate::from_string("a --fantasy scene with --magic").unwrap();
        assert_eq!(result.prompt, "a --fantasy scene with --magic");
        
        let result2 = Generate::from_string("test -xyz something").unwrap();
    ...
    fn test_negative_mode_with_unknown_options() {
        ...
        let result = Generate::from_string("scene --no --weird -stuff").unwrap();
    ...
    fn test_complex_real_world_example() {
        ...
        let result = Generate::from_string(input).unwrap();
        ...
        assert_eq!(result.negative_prompt, Some("blur text watermark".to_string()));
    }

    #[test]
    fn test_options_after_negative_mode() {
        // Options after --no should still be parsed as options
        let result = Generate::from_string("test --no bad stuff -w 100").unwrap();
        assert_eq!(result.prompt, "test");
